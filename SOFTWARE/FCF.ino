...
const float alpha = 0.6; // Low-pass filter smoothing factor
...
void loop() {
  ...
  float rawPitch = atan2(az, sqrt(ax * ax + ay * ay)) * 180.0 / PI - pitchOffset;
  float rawRoll  = atan2(-ax, sqrt(ay * ay + az * az)) * 180.0 / PI - rollOffset;

  // Apply low-pass filter
  filteredPitch = alpha * rawPitch + (1.0 - alpha) * filteredPitch;
  filteredRoll  = alpha * rawRoll  + (1.0 - alpha) * filteredRoll;

  // Map filtered values to servo angles
  int angleOffsetX = constrain((int)mapFloat(filteredPitch, -40.0, 40.0, -40.0, 40.0), -40, 40);
  int angleOffsetY = constrain((int)mapFloat(filteredRoll, -40.0, 40.0, -40.0, 40.0), -40, 40);
